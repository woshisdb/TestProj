...
{
    public ArrangeA(PersonObj PersonObj,Obj obj):base(PersonObj,obj)
    {
        wastTime = true;
    }
    public override IEnumerator<object> Run(System.Action<Act> callback)
    {
        ...
        {
            var it = contract;
            if (it.CanSign(PersonObj))//能够签字
                arrange.Add(new CardInf(it.cardInf.title, it.cardInf.description, () =>
                    {
                        GameArchitect.get.GetModel<ContractModel>().SignContract(it,PersonObj);
                    }
                )
                );
        }
        yield return GameArchitect.gameLogic.AddDecision(PersonObj, new DecisionTex("选择合约", "选择一个合适的的合约开始活动", 
            arrange
        ));
        var act = new EndAct(PersonObj, Obj);
...
public class ArrangeContractAct : Activity
{
    ...
    {
        return new ArrangeA(PersonObj, obj);
    }
}
...
{
    public AddContractA(PersonObj PersonObj, Obj obj) : base(PersonObj, obj)
    {
        wastTime = true;
    }
    public override IEnumerator<object> Run(System.Action<Act> callback)
    {
        TC();
        PersonObj PersonObj = (PersonObj)this.PersonObj;
        Debug.Log("Arrange");
        ...
        yield return GameArchitect.gameLogic.AddDecision(PersonObj, new SelectTex("选择数目", "选择协议数目",
        ...
        //        ));
        //    }
        //    yield return AddDecision(PersonObj,new DecisionTex("选择交互对象","",objs));
        //    yield return nowContract.codeData.EditCodeSystem(PersonObj, intObj, x);
        //}
...
public class AddContractAct : Activity
{
    ...
    public override Act Effect(Obj obj, PersonObj PersonObj, List<WinData> winDatas = null, params object[] objs)
    {
        return GetActs( new AddContractA(PersonObj, obj), obj, PersonObj,winDatas,objs); ;
    }
}
/// <summary>
/// 取消合约
...
{
    public RemoveContractA(PersonObj PersonObj, Obj obj) : base(PersonObj, obj)
    {
        wastTime = true;
    }
    public override IEnumerator<object> Run(System.Action<Act> callback)
    {
        ...
        var contracModel = GameArchitect.get.GetModel<ContractModel>();
        //foreach(var card in contracModel.aContract.GetValueOrDefault(PersonObj))
        //{
...
public class RemoveContractAct : Activity
{
    ...
    public override Act Effect(Obj obj, PersonObj PersonObj, List<WinData> winDatas = null, params object[] objs)
    {
        return GetActs( new RemoveContractA(PersonObj, obj), obj, PersonObj,winDatas,objs); ;
    }
}